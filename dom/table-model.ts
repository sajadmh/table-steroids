 ...
 */
function defaultGetCellText(cell: HTMLTableCellElement) {
 ...
 */
function getColumnId(index: number) {
  return `column-${index}`;
}
 ...
 */
function ensureGridRow(grid: DOMTableGrid, rowIndex: number) {
  grid[rowIndex] ??= [];
 ...
 */
export function getCoordinateKey(rowId: string, columnId: string) {
...
export function buildDOMTableModel(table: HTMLTableElement, options: BuildDOMTableModelOptions = {}): DOMTableModel {
  ...
  const cellByCoordinate = new Map<string, DOMTableCell>();
  const copyValueByCoordinate = new Map<string, string>();
  let maxColumnCount = 0;
  ...
  rowElements.forEach((rowElement, rowIndex) => {
    ...
    Array.from(rowElement.cells).forEach((cellElement, cellIndex) => {
      ...
      maxColumnCount = Math.max(
        maxColumnCount,
        registerCellAliases(cell, grid, rowSpan, colSpan, copyValue, cellByCoordinate, copyValueByCoordinate),
      );
