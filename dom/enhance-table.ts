...
import type { CellCoordinates, Selection, SelectionBounds } from "../core/types.js";
 ...
 */
function restoreManagedCell(cellElement: HTMLTableCellElement, previousTabIndex: string | null) {
...
function getEventCell(
  ...
  table: HTMLTableElement,
  cellByElement: WeakMap<HTMLTableCellElement, DOMTableCell>,
) {
  if (!isElement(target)) {
    return null;
  }
 ...
 */
function shouldIgnoreTarget(target: EventTarget | null, cellElement: HTMLTableCellElement) {
  if (!isElement(target)) {
    return false;
  }
 ...
 */
function createSelection(start: CellCoordinates, end: CellCoordinates): Selection {
  return { start, end };
}
 ...
 */
function cloneSelections(selections: Selection[]) {
 ...
 */
function createClipboardHtmlCell(model: DOMTableModel, rowId: string, columnId: string) {
 ...
 */
function appendClipboardHtmlRows(bodyElement: HTMLTableSectionElement, selection: SelectionBounds, model: DOMTableModel) {
 ...
 */
function buildClipboardHtml(plan: ReturnType<typeof resolveCopySelection>, model: DOMTableModel) {
  if (!plan) {
    return undefined;
  }
  ...
  const indexedSelections = plan.selections
    ...
    .filter((selection): selection is SelectionBounds => selection !== null);
 ...
 */
function getMeasuredCellEdges(cell: HTMLTableCellElement, overlayHost: HTMLElement | null = null) {
 ...
 */
function cellIntersectsSelectionBounds(
  cell: DOMTableCell,
  bounds: SelectionBounds,
  rowIndexMap: Record<string, number>,
  columnIndexMap: Record<string, number>,
  ...
  return cell.aliases.some((alias) => {
    ...
    return (
      rowIndex >= bounds.minRow && rowIndex <= bounds.maxRow && columnIndex >= bounds.minColumn && columnIndex <= bounds.maxColumn
    );
  });
}
...
interface FrozenBandGeometry extends OverlayFrozenBand {
  viewportLeft: number;
 ...
 */
function measureGroup(cells: DOMTableCell[], overlayHost: HTMLElement | null): MeasuredEdges | null {
  if (cells.length === 0) {
    return null;
  }
 ...
 */
function measureViewportX(cells: DOMTableCell[]) {
  if (cells.length === 0) {
    return null;
  }
...
function resolveOverlayRadius(
  bounds: SelectionBounds,
  model: DOMTableModel,
  cornerRadii: OverlayCornerRadii | null,
  suppressLeft: boolean,
  suppressRight: boolean,
): OverlayCornerRadii | undefined {
  if (!cornerRadii) {
    return undefined;
  }
 ...
 */
function cellIsFrozen(cell: DOMTableCell, model: DOMTableModel) {
...
function getOverlayRects(
  ...
  frozenBand: FrozenBandGeometry | null = null,
): { frozen: Omit<OverlayRect, "key"> | null; scrolling: Omit<OverlayRect, "key"> | null } {
  ...
  const buildRect = (
    ...
    const stroke: EdgeStroke | undefined = spansBoundary
 ...
 */
function isStickyScrollOverflowValue(value: string | undefined) {
 ...
 */
function getStickyScrollportLeft(overlayHost: HTMLElement | null) {
 ...
 */
function getFrozenBandGeometry(model: DOMTableModel, overlayHost: HTMLElement | null): FrozenBandGeometry | null {
 ...
 */
function viewportXToOverlayContent(value: number, overlayHost: HTMLElement | null) {
  if (!overlayHost) {
    return value;
  }
 ...
 */
function isClippingOverflowValue(value: string | undefined) {
 ...
 */
function getElementOverflowStyles(element: HTMLElement) {
  const computedStyle =
    typeof window.getComputedStyle === "function"
 ...
 */
function intersectClipRects(first: OverlayClipRect, second: OverlayClipRect) {
  const nextClipRect = {
 ...
 */
function getOverlayHost(table: HTMLTableElement): HTMLElement | null {
 ...
 */
function getLocalClipRect(rect: DOMRect, overlayHost: HTMLElement): OverlayClipRect {
 ...
 */
function getOverlayClipRect(table: HTMLTableElement, overlayHost: HTMLElement | null = null): OverlayClipRect | null {
  const tableRect = table.getBoundingClientRect();
  const hostRect = overlayHost?.getBoundingClientRect() ?? null;
  const viewportClipRect: OverlayClipRect = {
 ...
 */
export function enhanceTable(table: HTMLTableElement, options: TableSpreadsheetOptions = {}): TableSpreadsheetHandle {
  const enhancedTable = table as EnhancedTableElement;
   ...
   */
  const getCopyPayload = (selections: Selection[], activeSelectionForPlan: Selection | null) => {
    ...
    return {
      plan,
      text: copySelectionToText(
        plan,
        model.rows,
        model.columns,
        (rowId, columnId) => model.copyValueByCoordinate.get(getCoordinateKey(rowId, columnId)) ?? "",
      ),
  ...
  const syncFocusState = (
    nextActiveSelection: Selection | null,
   ...
   */
  const copySelectionsToClipboard = async (selections: Selection[], activeSelectionForPlan: Selection | null) => {
   ...
   */
  const syncFocusableCells = (nextModel: DOMTableModel) => {
  ...
  const renderOverlay = () => {
    ...
    const baseSelectionRects: OverlayRect[] = [];
    const frozenSelectionRects: OverlayRect[] = [];

    selectionRanges.forEach((selection) => {
      const { frozen, scrolling } = getOverlayRects(
        selection,
        model,
        rowIndexMap,
        columnIndexMap,
    ...
    let baseDragRect: OverlayRect | null = null;
    let frozenDragRect: OverlayRect | null = null;
  ...
  const rebuildModel = (clearSelection = false) => {
    ...
    if (clearSelection) {
      ...
      const restoredState = restoreSelectionState(
        selectionSnapshot ?? { selections: [], activeSelection: null },
        model.rows,
        model.columns,
      );
   ...
   */
  const syncSelections = (nextSelections: Selection[], nextActiveSelection: Selection | null) => {
   ...
   */
  const applySelection = (selection: Selection, mode: DragMode, baseSelections: Selection[]) => {
    if (mode === "replace") {
    ...
    if (mode === "add") {
      const dedupedSelections = subtractSelection(
        baseSelections,
        selection,
        model.rows,
        model.columns,
   ...
   */
  const setIsDraggingDocument = (nextValue: boolean) => {
    if (nextValue) {
  ...
  const handleTablePointerDown = (event: PointerEvent) => {
    ...
    const mode: DragMode = isToggleSelection
 ...
 */
export function enhanceTables(root: ParentNode = document, options: EnhanceTablesOptions = {}): TableSpreadsheetCollectionHandle {
  const { selector = "table", ...tableOptions } = options;
  const tables = Array.from(root.querySelectorAll(selector)).filter(
