 ...
 */
function getMeasuredCellEdges(cell: HTMLTableCellElement) {
...
export function enhanceTable(table: HTMLTableElement, options: TableSpreadsheetOptions = {}): TableSpreadsheetHandle {
   ...
   */
  const startDragSelection = (selection: Selection, anchor: CellCoordinates, mode: DragMode, baseSelections: Selection[]) => {
    lastDragMoved = false;
    dragState = {
      anchor,
      selection,
  ...
  const handleCellClick = (event: MouseEvent) => {
    ...
    if (isAlreadySelected) {
      const nextSelections = subtractSelection(
        selectionRanges,
        nextSelection,
        model.rows,
        model.columns,
